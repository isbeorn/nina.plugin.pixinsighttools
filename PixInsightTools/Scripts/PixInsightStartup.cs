    ...
    internal class PixInsightStartup : PixInsightScript {
        ...
        public Task Run(IProgress<ApplicationStatus> progress, CancellationToken ct) {
            return Task.Run(async () => {
                var pixInsightProcessIsRunningNoLock = Process.GetProcessesByName("PixInsight")?.FirstOrDefault(x => x.MainWindowTitle == $"PixInsight ({pixInsightSlot})") != null;
                if (!pixInsightProcessIsRunningNoLock) {
